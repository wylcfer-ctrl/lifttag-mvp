...
class RoutesTestCase(unittest.TestCase):
    ...
    def _checklist_data(self, **overrides):
        """
        Standard 5-point checklist fields, all confirmed, plus a default
        PASS result — merge in overrides (e.g. result='FAIL',
        failure_reason='...') per call. Added 2026-08-18: the standalone
        /t/<tag_id>/check route now requires the same mandatory checklist
        as the session flow (approved requirement 5) — every test below
        that posts to this route needs these fields added, since a post
        missing them is now a genuinely different, correctly-rejected
        request, not the same request that used to succeed.
        """
        data = {
            "checked_by": "Alice", "lift_supervisor": "Bob", "result": "PASS",
    ...
    def test_quarantine_banner_is_prominent_on_asset_page(self):
        self.client.post(
            "/t/active-tag/check",
            data=self._checklist_data(checked_by="A", lift_supervisor="B", result="FAIL",
    ...
    def test_full_check_flow_fail_quarantines(self):
        resp = self.client.post(
            "/t/active-tag/check",
            data=self._checklist_data(checked_by="Alice", lift_supervisor="Bob", result="FAIL",
                                       failure_reason="Damage"),
    ...
    def test_pass_after_quarantine_keeps_quarantine_and_explains_no_release(self):
        ...
        resp = self.client.post(
            "/t/active-tag/check",
            data=self._checklist_data(checked_by="C", lift_supervisor="D", result="PASS"),
            follow_redirects=True,
        )
        body = resp.data.lower()
